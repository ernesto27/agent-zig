...
/// Send a full conversation history (non-streaming) and return the parsed response.
...
pub fn sendMessage(
    ...
    const body = try buildRequestBody(allocator, self.config.model, messages, tools, null, false, self.config.effort);
...
/// Stream an Anthropic Messages response into the shared StreamAccumulator.
...
fn handleSseEvent(
    ...
    if (std.mem.eql(u8, event_name, "content_block_start")) {
        ...
        const block_kind: client.StreamBlockType = if (std.mem.eql(u8, block_type, "tool_use")) .tool_use else if (std.mem.eql(u8, block_type, "thinking")) .thinking else .text;
