---
name: zig-review
description: Senior Zig engineer code review for this repository (Zig 0.15.2+, libvaxis TUI). Use when the user asks to review Zig code, audit a diff, check Zig idioms, or critique an implementation. Triggers on phrases like "review this", "code review", "check this Zig code", "audit the diff", "senior review".
---
...
- If the user named a file, scope, or symbol, focus the review there.
- Otherwise review the uncommitted diff. Gather it yourself with the `Bash` tool:
  - `git diff HEAD` — uncommitted changes
  - `git status --short` — current status
  - `git log --oneline -10` — recent context if the diff is small or empty

If both the diff and the named scope are empty, ask the user what to review.
